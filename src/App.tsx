...
function App() {
  const pokemons = useSelector<PokemonStateImmutable, List<Pokemon>>(
    (state: any) => state.get('pokemons'),
  ).toJS();

  const loading = useSelector<PokemonStateImmutable, boolean>((state: any) =>
    state.get('loading'),
  );
  const dispatch: ThunkDispatch<PokemonStateImmutable, unknown, AnyAction> =
    useDispatch();
